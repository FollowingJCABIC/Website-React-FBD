...
import { readSchoolDb } from "../server/school-db.mjs";
  ...
  const routes = [
    {
      ...
      name: "Home",
      access: "public",
      purpose: "Intro, daily liturgy, activities, sacred media, and article explorer (full only).",
    },
    {
      path: "#/art",
      name: "Art Hall",
...
function answerWithLocalFallback(question) {
  ...
  if (q.includes("pdf")) {
    return "Full users can browse the PDF library on Home and annotate uploaded PDF pages in the School whiteboard.";
  }

  if (q.includes("login") || q.includes("sign in") || q.includes("full")) {
    return "Full login is required for article/PDF library access and assistant access. Visitor unlocks member pages.";
  }

  return [
    "Main pages: `#/` (Home), `#/art`, `#/youtube`, `#/school`.",
